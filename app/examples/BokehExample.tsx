...
function Circle() {
  ...
  const color = useSharedValue(getRandomHue());
  ...
  const update = () => {
    ...
    color.value = withTiming(color.value + getRandomHueDiff(), config);
  };
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    return {
      backgroundColor: `hsl(${color.value}, 100%, 50%)`,
      width: size,
      height: size,
